...
from src.utils.highlited_print import bcolors, print_warning, print_highlighted
...
class PTWTrainer(Trainer):
    ...
    def __init__(self, embed_args: EmbedArgs, env_args: EnvArgs):
        ...
        self.generator: ImageGenerator = None
    ...
    def train(self,
              generator: ImageGenerator,  # the generator that should be watermarked with a embedded_message.
              wm_key: WatermarkingKey):
        """ Embed the watermark into a generator.
        """
        # sample a new message:
        wm_key.wm_key_args.message = ''.join(np.random.choice(list(WatermarkingKeyArgs.ALPHABET), size=100))
        ...
        opt = Adam(generator.parameters(),  # only the synthesis layers.
        ...
        with tqdm(total=self.env_args.log_every, desc="Training PTW Key (infinity loop)") as pbar:
            while True:  # infinity loop
                ...
                with torch.no_grad():
                    w_frozen, x_frozen = reference_generator.generate(num_images=self.env_args.batch_size, seed=seed)

                _, x_train = generator.generate(w=w_frozen, seed=seed,
                                                use_gradients_after=5)
                ...
                loss_dict['capacity'] = max(0,
                                            2. * (
                                                    bit_acc.avg * wm_key.wm_key_args.bitlen - 0.5 * wm_key.wm_key_args.bitlen))
                loss_dict["bit_acc"] = bit_acc.avg

                #
                if step == 0:  # Check if gradients are available
                    any_gradient_unavailable = False
                    for param in generator.parameters():
                        if param.grad is None:
                            any_gradient_unavailable = True

                    if any_gradient_unavailable:
                        print_warning(f"> Some gradients were not available. Could be a bug!.")
                    else:
                        print_highlighted(f"> Successfully passed gradient checks. Seems to work!")
